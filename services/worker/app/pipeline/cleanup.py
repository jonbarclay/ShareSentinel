...
# Child event IDs: {parent}:child:{index} where parent is hex64 or UUID
...
def _extract_parent_hex(event_id: str) -> str | None:
    """Return the directory name for *event_id*, or ``None`` if invalid.

    Accepts parent IDs (``<hex64>`` or UUID) and child IDs
    (``<hex64>:child:<n>`` or ``<uuid>:child:<n>``).
    """
