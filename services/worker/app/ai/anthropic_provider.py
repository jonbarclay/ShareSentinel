...
class AnthropicProvider(BaseAIProvider):
    ...
    async def analyze(self, request: AnalysisRequest) -> AnalysisResponse:
        ...
        try:
            ...
            logger.warning("Anthropic transient error: %s", exc)
            raise TransientAIError(str(exc)) from exc
        except anthropic.APIStatusError as exc:
            if exc.status_code >= 500:
                logger.warning("Anthropic server error %d: %s", exc.status_code, exc)
                raise TransientAIError(str(exc)) from exc
            logger.exception("Anthropic analysis failed (status %d)", exc.status_code)
            return AnalysisResponse(
                categories=[], context="mixed", summary="", recommendation="",
                raw_response="", provider="anthropic", model=self.model,
