...
class EventRepository:
    ...
    async def create_event(self, job: Any) -> Optional[int]:
        ...
        Returns ``None`` if the event already exists and is NOT in
        'queued' status (i.e. already processed or currently processing).

        If the event exists with status='queued' (from a reprocessing
        script), it is claimed by transitioning to 'processing'.
    ...
    async def update_transcript_metadata(
        ...
        """Update transcript-related columns on the event."""
        sets: list[str] = ["updated_at = NOW()"]
        params: list[Any] = []
        idx = 1
        if transcript_source is not None:
            sets.append(f"transcript_source = ${idx}")
            params.append(transcript_source)
            idx += 1
        if media_duration_seconds is not None:
            sets.append(f"media_duration_seconds = ${idx}")
            params.append(media_duration_seconds)
            idx += 1
        if len(params) == 0:
            return
        params.append(event_id)
    ...
    async def requeue_event(self, event_id: str, failure_reason: str) -> int:
        """Reset an event to 'queued' for reprocessing.

        Increments retry_count, clears processing timestamps, and records
        the failure reason.  Returns the new retry_count.
        """
        async with self._pool.acquire() as conn:
            row = await conn.fetchrow(
                """
            ...
            new_count = row["retry_count"] if row else 0
            logger.info("Requeued event_id=%s retry_count=%d reason=%s", event_id, new_count, failure_reason)
            return new_count

    async def get_stuck_processing_events(self, timeout_minutes: int) -> List[Dict[str, Any]]:
        """Find non-child events stuck in 'processing' longer than *timeout_minutes*."""
        async with self._pool.acquire() as conn:
            rows = await conn.fetch(
                """
                SELECT event_id, operation, workload, user_id, object_id,
                       site_url, file_name, relative_path, item_type,
                       sharing_type, sharing_scope, sharing_permission,
                       event_time, retry_count, processing_started_at
                FROM events
